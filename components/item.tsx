...
const Price = styled(Span)`
    ...
    text-decoration: ${(props: PriceProps) => props.old ? 'line-through': 'none'};
    color: var(--text-color);
...
const SaleItem = ({ imageUrl, style, price, oldPrice, name, currency='gel' }: Props) => {
        ...
        <>  
            <Link href="/detail">
                <Wrapper>
                    ...
                    <div style={{display: 'flex', alignItems: 'flex-start', margin: '1.4rem 0'}}>
                        <Price style={{marginRight: '1.5rem'}} currency={currency}>{price} {CurrencyMap[currency]}</Price>
                        <Price old currency={currency}>{oldPrice} {CurrencyMap[currency]}</Price>
                    </div>
                    <Title>{name}</Title>
                </Wrapper>
            </Link>
